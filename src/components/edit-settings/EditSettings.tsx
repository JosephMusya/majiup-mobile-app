...
export default function EditSettings({ navigation }: any) {
    ...
    const { devices, ipAddress, setTanks, tanks } = useDeviceContext();
    ...
    const isTank = tanks.some((dev) => {
        return dev.id === tank.id;
    });
            ...
            try {
                const responseMetaData = await axios.post(
                    `${backendUrl}/tanks/${selectedDevice?.id}/profile`,
                    {
                        ...
                        receivenotifications:
                            changedMetaInfo.metaData.receivenotifications,
                        notifications: {
                            ...selectedDevice?.meta.notifications,
                        },
