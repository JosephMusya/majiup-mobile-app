...
export default function SettingsCard({
    ...
    const { pumps } = useDeviceContext();

    const isPump = pumps.some((pump) => {
        return pump.id === id;
    });
    ...
    return (
        <View style={[styles.container]}>
            <View
                style={[
                    styles.flexSpace,
                    { padding: 8, backgroundColor: "#d6d6d6" },
                ]}
            >
                <View>
                    <Text
                        style={{ fontSize: fontSize.xlarge, fontWeight: "500" }}
                    >
                        {name}
                    </Text>
            ...
            <View style={[styles.flexSpace, { padding: 8 }]}>
                ...
                {meta?.profile.address ? (
                    ...
                    <Text style={{ color: color.secondaryColor }}>
                        Add Location for this {isPump ? "pump" : "tank"}
                    </Text>
                )}
            </View>
            ...
            {!sensors?.some?.(
                ...
                isPump ? null : (
                    <View
                        style={[
                            flex.row,
                            { gap: 5, padding: 8, alignItems: "center" },
                        ]}
                    >
                        <Feather
                            name="alert-triangle"
                            ...
                            {WaterLevelSensor?.meta.critical_max === 0 &&
                                WaterLevelSensor.meta.critical_min === 0 && (
                                    <Text style={{ color: "orange" }}>
                                        <Text>
                                            {WaterLevelSensor.name} alerts not
                                            configured properly
                                        </Text>
                                    </Text>
                                )}
